...
### Score View System (MusicXML + Verovio)
...
- **Dark theme**: Verovio SVG is adapted to the dark UI via CSS overrides in `src/styles/globals.css` (`.score-view-host`), NOT via Verovio option keys (which are unstable across versions). Highlighted notes get `.vrv-playing` → accent color.
